"""International remote job boards with free public APIs."""
...
@register("remoteok", "RemoteOK", group="jobs", target_label="—",
          help="کلمات کلیدی = تگ‌ها (مثلا seo, marketing). خالی = همه‌ی آگهی‌های جدید.")
def remoteok(src: SourceConfig, limit: int) -> List[Item]:
    items, seen = [], set()
    for tag in src.queries() or [""]:
        ...
        for job in data if isinstance(data, list) else []:
            ...
            items.append(Item(
                ...
                salary=_salary(job.get("salary_min"), job.get("salary_max")), external_id=jid,
            ))
        pause(1)
    return items[:limit]


@register("remotive", "Remotive", group="jobs", target_label="—",
          help="کلمات کلیدی = عبارت جستجو (seo) یا دسته (category:marketing).")
def remotive(src: SourceConfig, limit: int) -> List[Item]:
    items = []
    for term in src.queries("seo"):
        ...
        if term.lower().startswith("category:"):
            ...
            params["search"] = term
        for job in get_json("https://remotive.com/api/remote-jobs", params=params).get("jobs", []):
            items.append(Item(
                title=job.get("title", ""), company=job.get("company_name", ""), url=job.get("url", ""),
...
@register("jobicy", "Jobicy", group="jobs", target_label="—",
          help="کلمات کلیدی = تگ (seo) یا صنعت (industry:marketing).")
def jobicy(src: SourceConfig, limit: int) -> List[Item]:
    items = []
    for term in src.queries("seo"):
        ...
        if term.lower().startswith("industry:"):
            ...
            params["tag"] = term
        for job in get_json("https://jobicy.com/api/v2/remote-jobs", params=params).get("jobs", []) or []:
            job_type = job.get("jobType") or []
            industry = job.get("jobIndustry") or []
            items.append(Item(
                title=strip_html(job.get("jobTitle", "")), company=job.get("companyName", ""),
                url=job.get("url", ""), location=job.get("jobGeo") or "Remote",
...
def _himalayas_item(job: dict) -> Item:
    ...
    return Item(
        ...
        posted_at=to_iso(job.get("pubDate")), job_type=job.get("employmentType") or "",
...
@register("himalayas", "Himalayas", group="jobs", target_label="—",
          help="کلمات کلیدی = عبارت جستجو (seo, digital marketing).")
def himalayas(src: SourceConfig, limit: int) -> List[Item]:
    items = []
    for term in src.queries("seo"):
        try:
            jobs = get_json("https://himalayas.app/jobs/api/search", params={"q": term}).get("jobs", [])
...
@register("arbeitnow", "Arbeitnow (اروپا)", group="jobs", target_label="—", keyword_mode="filter",
          help="همه‌ی آگهی‌های جدید؛ کلمات کلیدی برای فیلتر.")
def arbeitnow(src: SourceConfig, limit: int) -> List[Item]:
    items = []
    for job in get_json("https://www.arbeitnow.com/api/job-board-api").get("data", [])[:limit]:
        tags = list(job.get("tags") or []) + list(job.get("job_types") or [])
        if job.get("remote"):
            tags.append("Remote")
        items.append(Item(
            title=job.get("title", ""), company=job.get("company_name", ""), url=job.get("url", ""),
            location=job.get("location") or "", description=strip_html(job.get("description")),
            posted_at=to_iso(job.get("created_at")), job_type=", ".join(job.get("job_types") or []),
            tags=tags, external_id=str(job.get("slug") or job.get("url")),
        ))
    return items
